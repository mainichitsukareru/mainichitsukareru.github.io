...
function updateGrid100(percentage) {
    ...
    cells.forEach((cell, index) => {
        if (anime) {
            ...
            anime({
                targets: cell,
                fill: index < cellsToFill ? '#FF6B6B' : '#F7F9FC',
                duration: 300,
            ...
            cell.setAttribute('fill', index < cellsToFill ? '#FF6B6B' : '#F7F9FC');
        }
    });
}
...
function createGlassSVG() {
    ...
    // Glass container (trapezoid shape)
    const glass = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    glass.setAttribute('d', 'M 80 50 L 220 50 L 200 450 L 100 450 Z');
    ...
    liquid.setAttribute('d', 'M 100 450 L 200 450 L 200 450 L 100 450 Z');
...
function updateGlassLiquid(percentage) {
    ...
    // Calculate liquid height (0-400px range, from bottom)
    const maxHeight = 400;
    const height = (percentage / 100) * maxHeight;
    const topY = 450 - height;

    // Calculate trapezoid width at this height
    const bottomWidth = 100; // width at y=450
    const topWidth = 140; // width at y=50
    const ratio = (450 - topY) / 400;
    ...
    const path = `M 100 450 L 200 450 L ${rightX} ${topY} L ${leftX} ${topY} Z`;
